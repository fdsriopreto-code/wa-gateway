...
| Node | Pra quê |
|---|---|
| **wa-gateway** | Enviar texto/imagem/documento/vídeo/áudio/localização/**enquete**/**cartão de contato**, **resultado da enquete** (placar), **botões e templates (Cloud API)**, reagir, encaminhar, criar/gerenciar sessões, grupos, contatos (incl. **listar agenda**), **etiquetas do Business**, ler histórico. **Mencionar usuário em grupo** (`@número` no texto + campo Mencionar) em texto/imagem/documento/vídeo/áudio/enquete. Aceita **binário do nó anterior** (converte pra base64 sozinho). |
